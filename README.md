...
2. **Edge System:** Navigate to the `edge-raspberrypi` directory to run the upcoming Python scripts on the Raspberry Pi.
3. **Mobile App:** Run `flutter pub get` and `flutter run` inside the `mobile-client` directory once the application development commences.
